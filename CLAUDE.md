...
Strictly follow [Conventional Commits](https://www.conventionalcommits.org/) (git cz / commitizen format):
...
- **scope** (optional): module or area affected, e.g. `server`, `tools`, `stdio`, `settings`, `search`, `navigation`, `refactor`, `vcs`
- **subject** (required): imperative, lowercase, no period at the end
- **body** (optional): motivation and context for the change
- **footer** (optional): `BREAKING CHANGE: <description>` or issue references like `Closes #123`

Examples:
```
feat(tools): add GetTypeHierarchyTool for class hierarchy navigation
fix(server): handle concurrent SSE session cleanup on disconnect
refactor(stdio): switch from HTTP forwarding to TCP socket transport
docs: update README with current architecture and port configuration
build: bump IntelliJ platform target to 2025.1
```
